...
pub struct ComplexBuilder {
    ...
    cached_mode : Option<StepperDriveMode>,
...
impl ComplexBuilder {
    ...
    pub fn stop_with_mode(&mut self, mode : StepperDriveMode) {
        self.mode = StepperDriveMode::Stop;
    ...
    fn set_drive_mode<C : StepperController>(&mut self, mode : StepperDriveMode, ctrl : &mut C) -> Result<(), ActuatorError> {
        match mode {
            StepperDriveMode::ConstVelocity(mut velocity) => {
                ...
                if self.mode != StepperDriveMode::Inactive {
                    // Turn around motor
            ...
            StepperDriveMode::ConstFactor(_, dir) => {
                if (self.mode != StepperDriveMode::Inactive) & (dir != self._dir) {
                    // Turn around motor
                    self.stop_with_mode(mode.clone());
                } else {
                    self._dir = dir;
