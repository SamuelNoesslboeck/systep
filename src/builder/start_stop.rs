...
use syact::DefinedActuator;
use syact::data::ActuatorVars;

use crate::{StepperData, StepperConfig, StepperController, MicroSteps};
use crate::builder::AdvancedStepperBuilder;

use super::{StepperDriveMode, StepperBuilder, ActuatorError};
...
impl Iterator for StartStopBuilder {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.mode {
            ...
            StepperDriveMode::ConstVelocity(velocity ) => Some(velocity),
            // Drive a constant factor of the maximum possible velocity
            StepperDriveMode::ConstFactor(factor, _) => Some(self.velocity_possible() * factor),
            ...
            StepperDriveMode::FixedDistance(_, _, factor) => {
                ...
                if self.distance_counter > self.distance {
                    self.mode = StepperDriveMode::Inactive;
                    None
                } else {
            ...
            StepperDriveMode::Stop => {
                self.mode = StepperDriveMode::Inactive;
                None
            },
            // Inactive, no more nodes needed
            StepperDriveMode::Inactive => None
...
impl StepperBuilder for StartStopBuilder {
    ...
    fn set_drive_mode<C : StepperController>(&mut self, mode : StepperDriveMode, ctrl : &mut C) -> Result<(), ActuatorError> {
        match mode {
            // Driving with a constant velocity, check if the velocity is possible, return error if it is not
            StepperDriveMode::ConstVelocity(mut velocity) => {
            ...
            StepperDriveMode::ConstFactor(_, dir) => {
                self._direction = dir;
                ctrl.set_dir(dir)?;
            },
            // Check if the exit velocity is possible, everything else is fine
            StepperDriveMode::FixedDistance(rel_dist, velocity_exit, _) => {
...
// Extension traits
    impl AdvancedStepperBuilder for StartStopBuilder {
        // General constructors
            fn new(consts : StepperData, config : StepperConfig) -> Result<Self, ActuatorError>
            where 
                Self: Sized 
            {
                let mut _self = Self {
